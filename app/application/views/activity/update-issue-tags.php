		...
		<div class="tag-activity">
			...
			if($tag_counts['added'] > 0) { 
				foreach($tag_diff['added_tags'] as $tag) {
					echo '<label class="label"' . ($tag_diff['tag_data'][$tag]['bgcolor'] ? ' style="background: ' . $tag_diff['tag_data'][$tag]['bgcolor'] . '"' : '') . '>' . $tag_diff['tag_data'][$tag]['tag'] . '</label>';
				}
			...
			if($tag_counts['removed'] > 0) {
				foreach($tag_diff['removed_tags'] as $tag) {
					echo '<label class="label"' . ($tag_diff['tag_data'][$tag]['bgcolor'] ? ' style="float: none; background: '.$tag_diff['tag_data'][$tag]['bgcolor'].'"' : '').'>'.$tag_diff['tag_data'][$tag]['tag'].'</label>';
				}
